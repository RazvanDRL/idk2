...
function App() {
  ...
  fs.writeFile('entrants.txt', blockchain.account)
  for (let index = accs.length; accs !== ""; index--) {
    accs[index] = fs.writeFile('entrants.txt', blockchain.account);
  }
  var entrants = fs.readFile('entrants.txt', 'utf-8')
  function getRaffleWinners() {
    winners = entrants[Math.floor(Math.random() * entrants.length)];
  }
  setInterval(getRaffleWinners, 14400000);
  useEffect(() => {
    getData();
  }, [blockchain.account]);
